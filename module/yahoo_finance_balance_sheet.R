...
yahoo_finance_balance_sheet_ui <- function(id) {
...
PURI.JK", height = 250, width = 260),
                              ...
                              textAreaInput(ns("get_var_financial"), "Variable(s)", value = "Total Assets\nTotal Liabilities Net Minority Interest\nTotal Equity Gross Minority Interest\nTotal Capitalization\nCommon Stock Equity\nNet Tangible Assets\nWorking Capital", height = 150, width = 260),
...
yahoo_finance_balance_sheet_server <- function(input, output, session) {
  ...
  #  if(tes == 1)
   ...
   get_var = unlist(get_var)
    ...
    for(k in 1 :  length(company) )
    {
     ...
     # alamat <- paste0('https://finance.yahoo.com/quote/',company[k],'/financials?p=',company[k]) # income statement
      
      
      alamat <- paste0('https://finance.yahoo.com/quote/',company[k],'/balance-sheet?p=',company[k]) #balance sheet
  ...
  output$cetak_excel_ya <- renderText({
    ...
    for(k in 1 :  length(company) )
    {
      ...
      if(k == length(company))
      {
        ...
        openxlsx::addWorksheet(wb, "Balance Sheet", gridLines = TRUE)
        
        openxlsx::writeDataTable(wb, "Balance Sheet", simpan_dframe2, rowNames = FALSE, startRow = 2, startCol = 2, tableStyle = "TableStyleMedium21")
